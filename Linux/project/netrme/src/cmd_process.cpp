...
void DrawIntro(ArbotixPro *arbotixPro)
{
	...
	*/
	...
	if(Action::GetInstance()->LoadPage(indexPage, &Page)==TRUE)
 		printf("Success Load %d first\n",indexPage);
	else
		printf("Failed Load %d first\n",indexPage);
...
void PlayCmd(ArbotixPro *arbotixPro,int pageNum)
{
	...
	if(pageNum != indexPage)
		...
		if(Action::GetInstance()->LoadPage(indexPage, &Page) != true)
				{
                                ROS_INFO("Failed Load");
				memcpy(&Page,&page,sizeof(Action::PAGE));
				indexPage = oldIndex;
				return;
				}
                else
                        ROS_INFO("Success Load %d",indexPage);
		}
        else
		 ROS_INFO("Success Load indexPage");
	/*if(pageNum != indexPage)
		{
		memcpy(&page,&Page,sizeof(Action::PAGE));
		indexPage = pageNum;
	...
	if(Action::GetInstance()->Start(pageNum, &Page) == false)
	{
		...
		ROS_INFO("Failed to play this page!");
