...
extern ArbotixPro arbotixPro;
extern LinuxMotionTimer linuxMotionTimer;

void PopulateStepCurrent(ArbotixPro *arbotixPro, Action::STEP *CurrentStep )
{
	int value;
...
void SetTorque(ArbotixPro *arbotixPro, int ServoID, int on)
{
	if ( arbotixPro->WriteByte(ServoID, AXDXL::P_TORQUE_ENABLE, on, 0) != ArbotixPro::SUCCESS  )
	{
		printf( "Warning: SetTorque[%d] did not return success for servo: %d\r\n", on, ServoID );
	}
#ifdef DEBUG
	int value;
	if(arbotixPro->ReadWord(ServoID, AXDXL::P_PRESENT_POSITION_L, &value, 0) != ArbotixPro::SUCCESS)
	{
...
void TorqueAllServos(Robot::ArbotixPro *arbotixPro, char * input)
{
	int on;
	if(input[0]=='1')
	{
...
void GotoPose(ArbotixPro *arbotixPro, char* input )
{
	...
	PopulateStepCurrent(arbotixPro,&CurrentStep);
...
void GetCurrentPose( ArbotixPro *arbotixPro, char* response )
{
	Action::STEP CurrentStep;
	PopulateStepCurrent( arbotixPro,&CurrentStep );
...
void SetServo( ArbotixPro *arbotixPro, char* input )
{
	//SETSERVO:1:774
	input+=9;
	...
	{		
		...
		int error;
		if ( arbotixPro->WriteWord(index, AXDXL::P_GOAL_POSITION_L, value, &error) != ArbotixPro::SUCCESS )
		{
...
void SetServoSpeeds( ArbotixPro *arbotixPro, char* input )
{
	//SETSPEED:0-1023
	input+=9;
	...
	{		
		...
		int error;
		if ( arbotixPro->WriteWord(ArbotixPro::ID_BROADCAST, AXDXL::P_MOVING_SPEED_L, speed, &error) != ArbotixPro::SUCCESS )
		{
...
void PlayPage( ArbotixPro *arbotixPro, char* input )
{
	...
	MotionManager::GetInstance()->SetEnable(false);
	linuxMotionTimer.Stop();
	ROS_INFO("Play sucess");
	//GetCurrentPose( arbotixPro, tempMessage );
...
void UDP_Command_Handler( char * p_udpin )
{
	...
	{
		TorqueAllServos( &arbotixPro, p_udpin+10 );
	...
	else if( memcmp( p_udpin, "TORQUE", 6 ) == 0 )
	{
		...
		tok = strtok( 0, " :" );
		int onoff = atoi( tok );
		SetTorque( &arbotixPro, servoid, onoff );
		GetCurrentPose( &arbotixPro, tempMessage );
	}
	else if( memcmp( p_udpin, "GOTOPOSE:", 9 ) == 0 )
	{
		GotoPose( &arbotixPro, p_udpin );
		return;
	}
...
//NOTE: For safety, especially during debugging, make sure motor torque is on before playing!
	...
	{
		SetServo( &arbotixPro, p_udpin );
		return;
	}
	else if( memcmp( p_udpin, "SETSPEED:", 9 ) == 0 )
	{
		SetServoSpeeds( &arbotixPro, p_udpin );
		return;
	}
...
void UDP_Control_Handler( char * p_udpin )
{
	...
	if ( memcmp( p_udpin, "BIND", 4 ) == 0 ) 
	{
		...
		if ( arbotixPro.WriteWord(ArbotixPro::ID_BROADCAST, AXDXL::P_TORQUE_LIMIT_L, 684, 0) != ArbotixPro::SUCCESS  )
		{
			printf( "Warning: TODO\r\n");
		}
		if ( arbotixPro.WriteWord(ArbotixPro::ID_BROADCAST, AXDXL::P_MOVING_SPEED_L, 256, 0) != ArbotixPro::SUCCESS )
		{
			printf( "Warning: TODO\r\n");
		}
	}
